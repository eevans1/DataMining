...
colnames(med_val_lr) <- c("green_rating", "leasing_rate")
g3 <- ggplot(data = subset(gb_cut, new == 1), aes(x = factor(green_rating), y = leasing_rate)) + 
  geom_boxplot(aes(colour = factor(green_rating))) + ylab("Leasing rate (%)") +
  scale_colour_manual(name = "",
                      guide = FALSE,
                      values = c("0" = "#5a5a5a","1" = "#4daf4d")) + 
...
colnames(med_val_r) <- c("green_rating", "Rent")
g4 <- ggplot(data = subset(gb_cut, new == 1), aes(x = factor(green_rating), y = Rent)) + 
  geom_boxplot(aes(colour = factor(green_rating))) + ylab("Rent ($/sqft)") +
  scale_colour_manual(name = "",
                      guide = FALSE,
                      values = c("0" = "#5a5a5a","1" = "#4daf4d")) + 
  ...
  geom_text(data = med_val_r, aes(x = factor(green_rating), y = Rent, label = Rent), 
            size = 3, vjust = -1)
grid.arrange(g3,g4,nrow=2, padding = unit(0, "line"))
...
colnames(med_val_lr) <- c("green_rating", "leasing_rate")
g5 <- ggplot(data = comps_df, aes(x = factor(green_rating), y = leasing_rate)) + 
  geom_boxplot(aes(colour = factor(green_rating))) + ylab("Leasing rate (%)") +
  scale_colour_manual(name = "",
                      guide = FALSE,
                      values = c("0" = "#5a5a5a","1" = "#4daf4d")) + 
...
colnames(med_val_r) <- c("green_rating", "Rent")
g6 <- ggplot(data = comps_df, aes(x = factor(green_rating), y = Rent)) + 
  geom_boxplot(aes(colour = factor(green_rating))) + ylab("Rent ($/sqft)") +
  scale_colour_manual(name = "",
                      guide = FALSE,
                      values = c("0" = "#5a5a5a","1" = "#4daf4d")) + 
  ...
  geom_text(data = med_val_r, aes(x = factor(green_rating), y = Rent, label = Rent), 
            size = 3, vjust = -1)
grid.arrange(g5,g6,nrow=2, padding = unit(0, "line"))
```

Ultimately what one the two positive outcomes that a developer is looking for is (a) rent and (b) leasing rate. We simply plot our data on these two axis to get an idea of how the data set is distributed
...
We note here that the buildings with green certifications appear to have slightly higher leasing rates. Furthermore, there appears to be a larger clump of low-rent buildings without green certificaions
```{r echo=FALSE}
med_val_lr <- ddply(gb_cut, .(green_rating), summarise, med = median(leasing_rate))
colnames(med_val_lr) <- c("green_rating", "leasing_rate")
g1 <- ggplot(data = gb_cut, aes(x = factor(green_rating), y = leasing_rate)) + 
...
It is difficult to make out everything so we will cycle through the different states
```{r echo=FALSE}
ani_cat <- animate(g_shaped + transition_manual(cat), renderer = gifski_renderer(), width = 1200, height = 800, res = 100)
ani_cat
```
...
To finalize this study we can control for a few more variables that the situation describes. The developer describes the building that is, 15 stories, Mixed use, Is located on East Cesar Chavez
One can find online sources for the approximate rent of an East Cesar Chavez apartment. Estimates are approximately $27/sqft and is newly constructed.
With these qualifications we can find apartments in our data set that could be considered roughly comprable
...
Data is now shown in a box plot
```{r echo=FALSE}
d_val_lr <- ddply(comps_df, .(green_rating), summarise, med = median(leasing_rate))
colnames(med_val_lr) <- c("green_rating", "leasing_rate")
...
ggplot(data = ABIA, aes(x = factor(CarrierName), y = TotalDelay)) + 
  geom_boxplot(aes(colour = factor(CarrierName))) + ylab("Delay (mins)") + 
