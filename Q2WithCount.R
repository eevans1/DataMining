...
#Counting
count(DenFlights, vars=DenFlights$UniqueCarrier)
FlightVolume <- count(DenFlights, vars=DenFlights$UniqueCarrier)

#Plotting by Mean
AvgCarrierDelay <- aggregate(DenFlights$TotalDelay ~ DenFlights$UniqueCarrier, data=DenFlights, mean) # finding mean by carrier
AvgCarrierDelay$Volume <- FlightVolume$n ##Warning message: Unknown or uninitialised column: 'freq'. 
...
ggplot(data=AvgCarrierDelay, aes(x=AvgCarrierDelay$Carrier)) +
  geom_bar(aes(y = AvgCarrierDelay$AvgDelay), stat="identity", width=.5, fill="tomato3") +
